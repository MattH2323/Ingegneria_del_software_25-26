...
- `<hands_on2/>` :
    ...
    - `usecase_diagram.drawio`: file xml del use case diagram utilizzato nel srs

- `<hands_on3/>` :
    - ` *.drawio` : file xml relativo all'uml
    - ` *.jpg` : file jpg (immagine) relativo dell'uml

- `<hands_on4/>` :
    - `after/` : documentazione delle metriche e statistiche finali post-intervento
        - `out/` : report HTML dettagliati relativi alla test coverage finale
